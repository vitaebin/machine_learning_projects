...
class LearningAgent(Agent):
    ...
    def get_maxQ(self, state):
        ...
        for aa in self.valid_actions:
            if self.Q[state][aa]>=qvalue:
                qvalue = self.Q[state][aa]
        maxQ=qvalue
            ...
            if pp < self.epsilon:
               action = self.valid_actions[random.randint(0, len(self.valid_actions)-1)] 
                ...
                for aa in self.valid_actions:
                    if self.Q[state][aa]== qvalue:
                        aalist.append(aa)
                action = aalist[random.randint(0, len(aalist)-1)]
...
def run():
    ...
    sim = Simulator(env, log_metrics=True, optimized=True, update_delay=0.01)
